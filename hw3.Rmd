...
### c
```{r}
hist(a_sample, main = "Histogram of samples from Exp(1)")
```
...
hist(d_normalized, main = "Sample Means from Exp(1) with n = 100")
```
It looks like the standard normal because before the normalization, the sample means are approximately normal distributed. After normalizing them by subtracting the mean and divided by the variance, it should follow approximate standard normal. It is also stated by the CLT.
...
f <- c(f_mean, f_variance)
names(f) <- c("Mean of the sample variance", "Variance of the sample variance")
print(f)
```
